...
//  textFont(font);                ///tao phong chu
  ...
  text("First demo IMU by GGAIN", 30, 30);
  ...
  text("angle x: "+angle_x,300,70); 
  text("angle y: "+angle_y,300,85); 
  text("angle z: "+angle_z,300,100);
 
  text("accel angle x: "+degrees(accel_angle_x),450,70); 
  text("accel angle y: "+degrees(accel_angle_y),450,85); 
  //text("accel angle z: "+angle_z,10,200); 
  
  text("fil angle x: "+fil_angle_x,600,70); 
  text("fil angle y: "+fil_angle_y,600,85); 
 
  fill(255,255,255);
  textAlign(CENTER);
  text("Gyro angle",xbox2,8*ybox2/6);
  text("Complement filter",xbox1,8*ybox2/6);
  text("Accel angle",xbox3,8*ybox1/6);
  textAlign(LEFT);
  textSize(20);
//  rotateX(PI/3);
//  rotateZ(PI);
...
//  rotateY(ztheta);
  ...
  pushMatrix();
  translate(xbox3, ybox3, 0);        //cho ve giua man hinh
  rotateX(PI/2); 
  rotateY(accel_angle_y);
  rotateX(-accel_angle_x);
  ...
  pushMatrix();
  translate(xbox1, ybox1, 0);        //cho ve giua man hinh
  rotateX(PI/2); 
  rotateY(radians(fil_angle_y));
  rotateX(radians(-fil_angle_x));
  //rotateZ(radians(-angle_z));
  hop(200);
  popMatrix();
